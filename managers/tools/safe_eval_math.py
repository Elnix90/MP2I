...
def _safe_eval(node: ast.AST) -> float:
    ...
    if isinstance(node, ast.BinOp):
        if not isinstance(node.op, ALLOWED_BINOPS):
            raise ValueError("Operator not allowed")  # noqa: TRY004
    ...
    if isinstance(node, ast.UnaryOp):
        if not isinstance(node.op, ALLOWED_UNARYOPS):
            raise ValueError("Unary operator not allowed")  # noqa: TRY004
    ...
    if isinstance(node, ast.Call):
        if not isinstance(node.func, ast.Name):
            raise ValueError("Only direct function calls are allowed")  # noqa: TRY004
...
async def safe_eval_math(expression: str, precision: int = 10) -> str:
    ...
    try:
        ...
        logger.error("safe_eval_math failed: %s", exc)
