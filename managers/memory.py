...
def make_scope_key(
    *, channel_id: int | None = None, thread_id: int | None = None
    ...
    """Build the memory scope key for a conversation container.

    Threads scope their own memory; other channels (normal-mode) share one
    memory per channel.

    Parameters
    ----------
    channel_id : int | None
        Channel id for normal-mode scopes.
    thread_id : int | None
        Thread id for thread-mode scopes.

    Returns
    -------
    str
        Scope key used to filter stored turns.
    """
...
class MemoryTurn:
    ...
    def scope_key(self) -> str:
        """Return the scope key this turn belongs to."""
...
async def memory_lifespan(builder: coco.EnvironmentBuilder) -> AsyncIterator[None]:
    """CocoIndex lifespan that provides the memory services."""
...
async def memory_app_main() -> None:
    """Declare the turn table from the in-memory store."""
    ...
    turn_table = await coco_sqlite.mount_table_target(
        MEMORY_DB_KEY, "memory_turns", turn_schema
    )
...
class MemoryManager:
    """Scoped conversation history manager with CocoIndex persistence.

    ``max_history`` bounds how many recent turns are replayed per scope.
    """
    ...
    def __init__(
        ...
        *,
        ...
        ----------
        max_history : int
            Maximum number of recent turns to replay per scope. Default is 15.
        state_path : Path | str
    ...
    def _save_state(self) -> None:
        """Persist current turns to disk atomically."""
    ...
    def get_turn(self, turn_id: str) -> MemoryTurn:
        """Return a stored turn by id or prefix."""
    ...
    def list_turns(
        self, scope_key: str | None = None, limit: int = 10
    ) -> list[MemoryTurn]:
        """List recent turns, optionally filtered by scope.

        Parameters
        ----------
        scope_key : str | None
            Scope to filter by, or None for all scopes.
        limit : int
    ...
    def get_history(
        ...
        """Return a chat-style message history for a scope.

        User turns are tagged with the author display name so the model knows
        who said what.

        Parameters
        ----------
        scope_key : str
            Scope of the conversation to fetch.
        limit : int | None
            Maximum number of user turns to include (default: None for all
            that fit within ``max_history``).
        ...
        for turn in turns:
            messages.append(
                {"role": "user", "content": f"[{turn.user_name}]: {turn.user_content}"}
            )
    ...
    def clear_history(self, scope_key: str | None = None) -> int:
        """Clear history for a given scope or for all scopes.
        ...
        removed_ids = [
            tid for tid, t in self._turns.items() if t.scope_key() == scope_key
        ]
    ...
    async def sync(self) -> None:
        """Persist state and update the CocoIndex app."""
    ...
    async def clear_history_and_sync(self, scope_key: str | None = None) -> int:
        """Clear history for a scope (or all) and persist the change."""
        async with self._sync_lock:
...
def get_memory(max_history: int = 15) -> MemoryManager:
    """Return the process-wide MemoryManager, creating it on first use."""
