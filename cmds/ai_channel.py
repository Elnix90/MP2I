...
async def _update_channel(interaction: discord.Interaction, *, allow: bool):
    ...
    try:
        ...
        if channel is None:
            return await interaction.response.send_message(
                "Cette commande doit être utilisée dans un salon.", ephemeral=True
            )

        channels = load_allowed_channels()
        if allow and channel.id in channels:
            return await interaction.response.send_message(
                f"Le bot répond déjà dans <#{channel.id}>.", ephemeral=True
            )
        ...
        if not interaction.response.is_done():
            await interaction.response.send_message(
                f"Error while running /{command_name}.", ephemeral=True
            )
        else:
